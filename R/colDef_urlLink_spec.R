...
colDef_urlLink_spec <- function(cols, col_url, data){
  ...
  tmp_coldef_cols <- reactable::colDef(html = TRUE, cell = function(value, index) {
  ...
  named_list_of_colDefs_cols <- stats::setNames(list_of_colDefs_cols, cols)

  tmp_coldef_col_url <- reactable::colDef(show = F, cell = function(value, index) {})

  list_of_colDefs_col_url <- lapply(col_url, function(col) {return(tmp_coldef_col_url)})

  named_list_of_colDefs_col_url <- stats::setNames(list_of_colDefs_col_url, col_url)
